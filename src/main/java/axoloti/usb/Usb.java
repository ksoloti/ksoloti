        ...
        try {
            ...
            for (Device device : list) {
                ...
                if (result == LibUsb.SUCCESS) {
                    ...
                    else if (prefs.boards.isKsolotiDerivative() && descriptor.idVendor() == VID_AXOLOTI && descriptor.idProduct() == PID_KSOLOTI) {
                        hasOne = true;
                        DeviceHandle handle = new DeviceHandle();
                        result = LibUsb.open(device, handle);
                        if (result < 0) {
                    ...
                    else if (prefs.boards.isAxolotiDerivative() && descriptor.idVendor() == VID_AXOLOTI && descriptor.idProduct() == PID_AXOLOTI) {
                        hasOne = true;
                        DeviceHandle handle = new DeviceHandle();
                        result = LibUsb.open(device, handle);
                        if (result < 0) {
