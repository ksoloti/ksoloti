...
//        WriteAxoObject("patch", CreateInitMsg());
    ...
    static AxoObject modsource_cc() {
        ...
        o.sMidiCode = "if ((status == MIDI_CONTROL_CHANGE + attr_midichannel)&&(data1 == %cc%)) {\n"
                ...
                + "    &parent->GetModulationTable()[parent->MODULATOR_CGENATTR_instancename * NMODULATIONTARGETS],\n"
                + "    NMODULATIONTARGETS,\n"
                + "    &parent->PExch[0],\n"
                + "    &parent->PExModulationPrevVal[parent->polyIndex][parent->MODULATOR_CGENATTR_instancename],\n"
                + "    data2<<20);\n"
                + "}\n";
        return o;
    }
