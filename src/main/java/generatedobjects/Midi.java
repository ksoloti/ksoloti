...
public class Midi extends gentools {
    ...
    static AxoObject Create_keyb_mod() {
        ...
        o.sMidiCode = "if ((status == MIDI_NOTE_ON + %midichannel%) && (data2)) {\n"
                ...
                + "    &parent->GetModulationTable()[parent->MODULATOR_CGENATTR_instancename_velocity * NMODULATIONTARGETS],\n"
                + "    NMODULATIONTARGETS,\n"
                + "    &parent->PExch[0],\n"
                + "    &parent->PExModulationPrevVal[parent->polyIndex][parent->MODULATOR_CGENATTR_instancename_velocity],\n"
                + "    _velo<<20);\n"
                + "  PExModulationSourceChange(\n"
                + "    &parent->GetModulationTable()[parent->MODULATOR_CGENATTR_instancename_note * NMODULATIONTARGETS],\n"
                + "    NMODULATIONTARGETS,\n"
                + "    &parent->PExch[0],\n"
                + "    &parent->PExModulationPrevVal[parent->polyIndex][parent->MODULATOR_CGENATTR_instancename_note],\n"
                + "    _note<<21);\n"
                + "} else if (((status == MIDI_NOTE_ON + %midichannel%) && (!data2))||\n"
                + "          (status == MIDI_NOTE_OFF + %midichannel%)) {\n"
                + "  if (_note == data1-64) {\n"
                ...
                + "    _gate = 0;\n"
                + "  PExModulationSourceChange(\n"
                + "    &parent->GetModulationTable()[parent->MODULATOR_CGENATTR_instancename_releasevelocity * NMODULATIONTARGETS],\n"
                + "    NMODULATIONTARGETS,\n"
                + "    &parent->PExch[0],\n"
                + "    &parent->PExModulationPrevVal[parent->polyIndex][parent->MODULATOR_CGENATTR_instancename_releasevelocity],\n"
                + "    _rvelo<<20);\n"
                + "  }\n"
                + "} else if ((status == %midichannel% + MIDI_CONTROL_CHANGE)&&(data1 == MIDI_C_ALL_NOTES_OFF)) {\n"
                + "  _gate = 0;\n"
