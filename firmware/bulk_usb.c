...
typedef enum _BLType {
    blWrite,
    blStartTransmit, blStartReceive,
    blEndTransmit, blEndReceive,
    blFromNotify, blFromUSBInt,
    blConfigure, blNotifyCalled,
    blBlockedTX, blBlockedRX
} BLType;
...
static size_t write(void *ip, const uint8_t *bp, size_t n) {
    ...
    if ((usbGetDriverStateI(bdup->config->usbp) != USB_ACTIVE) || (bdup->state != BDU_READY)) {
        return 0;
    }
    ...
    return r;
#else
    return oqWriteTimeout(&((BulkUSBDriver *)ip)->oqueue, bp, n, TIME_INFINITE);
#endif
}

static size_t read(void *ip, uint8_t *bp, size_t n) {
    return iqReadTimeout(&((BulkUSBDriver *)ip)->iqueue, bp, n, TIME_INFINITE);
}
...
static const struct BulkUSBDriverVMT vmt = {
    .write  = (size_t (*)(void *, const uint8_t *, size_t))write,
    .read   = (size_t (*)(void *, uint8_t *, size_t))read,
    .put    = (msg_t (*)(void *, uint8_t))put,
    .get    = (msg_t (*)(void *))get,
    .putt   = (msg_t (*)(void *, uint8_t, sysinterval_t))putt,
    .gett   = (msg_t (*)(void *, sysinterval_t))gett,
    .writet = (size_t (*)(void *, const uint8_t *, size_t, sysinterval_t))writet,
    .readt  = (size_t (*)(void *, uint8_t *, size_t, sysinterval_t))readt
};
...
static void onotify(io_queue_t *qp) {
    ...
    if (!usbGetTransmitStatusI(bdup->config->usbp, bdup->config->bulk_in) &&
        ...
        //chSysUnlock();
        //CH16 usbPrepareQueuedTransmit(bdup->config->usbp, bdup->config->bulk_in, &bdup->oqueue, n);
        //chSysLock();
