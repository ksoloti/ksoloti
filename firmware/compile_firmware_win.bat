...
cd %axoloti_firmware%
make BOARDDEF=%1 -f Makefile.patch.mk clean axoloti_firmware="%FW_DIR%" axoloti_home="%HOME_DIR%"
...
make -j8 BOARDDEF=%1 axoloti_firmware="%FW_DIR%" axoloti_home="%HOME_DIR%"
if %ERRORLEVEL% neq 0 (
exit /b 1
)
...
make -j8 BOARDDEF=%1 FWOPTIONDEF=FW_SPILINK axoloti_firmware="%FW_DIR%" axoloti_home="%HOME_DIR%"
if %ERRORLEVEL% neq 0 (
exit /b 1
)
...
make -j8 BOARDDEF=%1 FWOPTIONDEF=FW_USBAUDIO axoloti_firmware="%FW_DIR%" axoloti_home="%HOME_DIR%"
if %ERRORLEVEL% neq 0 (
exit /b 1
)
...
make -j8 BOARDDEF=%1 FWOPTIONDEF=FW_I2SCODEC axoloti_firmware="%FW_DIR%" axoloti_home="%HOME_DIR%"
if %ERRORLEVEL% neq 0 (
exit /b 1
)
