...
void __attribute__((section(".framtext"))) FlashSystemReset(void)
{
...
static uint32_t __attribute__((section(".framtext"))) revbit(uint32_t data)
{
  uint32_t result;
  __ASM
...
uint32_t __attribute__((section(".framtext"))) FlashCalcCRC32(uint8_t *buffer, uint32_t size)
{
  uint32_t i, j;
  uint32_t ui32x;
