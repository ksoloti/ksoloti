 ...
 */
static bool msd_scsi_process_start_read_write_10(USBMassStorageDriver *msdp, uint8_t** buff) {
 ...
 */
static bool msd_scsi_process_read_format_capacities(USBMassStorageDriver *msdp) {
 ...
 */
static bool msd_wait_for_command_block(USBMassStorageDriver *msdp) {
 ...
 */
static bool msd_read_command_block(USBMassStorageDriver *msdp, uint8_t** buff) {
...
int msdStart(USBMassStorageDriver *msdp, const USBMassStorageConfig *config) {
    ...
    static THD_WORKING_AREA(mass_storage_thread_wa, 2048);
    /* run the thread */
    msdp->thread = chThdCreateStatic(mass_storage_thread_wa, sizeof(mass_storage_thread_wa), HIGHPRIO-1, (void*) mass_storage_thread, msdp);
